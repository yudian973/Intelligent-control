/*****************************************************************
* Filename     :  EM78P520PIN44.ASM
* Author       :  yu.wei
* Company      :  ELAN
* VERSION      :  4.0
* CRYSTAL      :  8MHZ
* Creat date   :  2009/12/18
...
;--------------------------------------------------------------------
	...
	XOR                 A,KeyCheckBakup
	JBS                 STATUS,Z
	JMP                 SearchSleep_Status_Judge_End1

	MOV                 A,SleepCNT
	SUB                 A,@SetSleepTime        ;
	JBC                 STATUS,C
	JMP                 SearchSleep_Status_Judge_End2
  ...
  SearchSleep_Status_Judge_End1:
	CLR                 SleepCNT
  SearchSleep_Status_Judge_End2:
	MOV                 A,TEMP
	MOV                 KeyCheckBakup,A	
;=====================================================================
...
Waiting_For_SearchData_Arrive:
	MOV                 A,ComuClock                  ; 8.192ms*ComuClock Interrupt ,
	SUB                 A,@0
	JBS                 STATUS,C
...
Write_EEprom_Task:
	...
	BC                  ForceLinkModeFlag/8,ForceLinkModeFlag%8
	BC                  FccTestModeFlag/8,FccTestModeFlag%8
  if WEEPROM_T_DEBUG & EEPROM_DEBUG & DebugDisplay == 1
	BANK                0
	MOV                 A,@0B00001000          ; (test)P83 exchange when intrrupt
	XOR                 PORT8,A
  endif
...
Sampling_Task:
	...
	BANK                5
	BS                  CMOS_PARAMETER,2      ; get COMS sensor data
	LCALL               CMOSSensor_Function

	;LCALL               GSSensor_Function
	NOP
	RET
