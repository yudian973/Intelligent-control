...
include "..\..\..\..\include\EM78xx\EM78CtrlIns.H"
endif

ifndef EM78Math.H
include "..\..\..\..\include\EM78xx\EM78Math.H"
endif
...
;========================== Iinitial config ================================
  if ADC_FUNCTION == 1
	...
	MOV                 A,@_DireInput ; 00110110
	MOV                 PBIOCR,A      ; PB5 PB4 PB2 PB1 As Input
	BANK                5
	MOV                 A,@11110000B
...
Pre_Discharge:
  ...
  if IOtoADC_T_DEBUG & DebugDisplay ==1
...
;---------------------------------------------------------------------
  Falling_Edge_Judge:
	...
	JBS                 KeyScanStatusFlag/8,KeyScanStatusFlag%8        ; scan the first falling edge
	BS                  KEY_NUM,0                            ; Will into key scan, Set guide bit
	CLR                 KeystokeTimeCNT                      ; press times ,recalculate
	BC                  KeyStatusFlag/8,KeyStatusFlag%8    ; key status, press(0) or release(1)
	BS                  KeyScanStatusFlag/8,KeyScanStatusFlag%8	     ; into scan key status
	JMP                 Edge_Judge_End
...
;------------------------------------------------------------------------------
  KeystokeScan_End:                                                ; a cycle scan end
	JBS                 KeyScanStatusFlag/8,KeyScanStatusFlag%8
	CLR                 KEY_NUM
	...
	RET
	BC                  KeyScanStatusFlag/8,KeyScanStatusFlag%8  ; more than 60*16ms=960ms ,EXIT scan status
	BS                  KeyScanFinishFlag/8,KeyScanFinishFlag%8
	RET
